...
let canWeDeliver = function (zipcode) {
...
const updatedDeals = deals.map((deal) => {
    ...
    return { ...deal, title: deal.title.replace('15%', '10%') }
    } else {
    return deal
    }
})
...
const updatedDeals1 = deals.map((deal) => {
    if (deal.title.includes('Free Kids')) {
    return { ...deal, desc: deal.desc.trim().replace('March', 'April') }
    } else {
    return deal;
    }
})

// console.log(updatedDeals1)